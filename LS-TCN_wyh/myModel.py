...
class Chomp1d(nn.Module):
    def __init__(self, chomp_size):
        super(Chomp1d, self).__init__()    # 对继承自父类属性进行初始化
        self.chomp_size = chomp_size

    def forward(self, x):
        """
        这是一个裁剪的模块，裁剪多出来的padding
        """
...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout):
        ...
        #     nn.Conv1d(n_outputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation))
        # self.chomp3 = Chomp1d(padding)  # 裁剪掉多出来的padding部分，维持输出时间步为seq_len
        # self.relu3 = nn.ReLU()
...
class TCN(nn.Module):
    def __init__(self, input_size=409, output_size=17, num_channels=[256, 256, 256, 256, 256, 256], kernel_size=20, DROP=0.2):
        super(TCN, self).__init__()
        self.tcn = TemporalConvNet(input_size, num_channels, kernel_size=kernel_size, dropout=DROP)
        self.linear1 = nn.Linear(num_channels[-1], output_size)
        # self.linear1 = nn.Linear(num_channels[-1], 32)
        # self.linear2 = nn.Linear(32, output_size)

    def forward(self, inputs):
        """Inputs have to have dimension (N, C_in, L_in)"""
        y1 = self.tcn(inputs)  # input should have dimension (batch, Channel, Length)
        net = self.linear1(y1[:, :, -1])
        # net1 = self.linear1(y1[:, :, -1])    # 增加一个维度，1D.FCN
        # net = self.linear2(net1)
