...
if __name__ == '__main__':
    ...
    parser.add_argument('--position', type=str, default='forearm')
    ...
    _, X, y = readmat('E:\\Study\\Data\\Session_1\\DataExtracted' + str(obj) + '.mat', winsize=WINSIZE, wininc=WININC,
                    pos=position, normalize=normalize)
    # tmp = torch.mean(X[0], axis=0) # 验证数据是否去中心化了，看tmp是不是0: e^-20, 就是0了
    from kfold import k_fold
    train_l, test_l, train_acc, test_acc, train_f1, test_f1 = k_fold(k=7, gpu=gpu, position=position, obj=obj,
                                                                     X=X, y=y,
