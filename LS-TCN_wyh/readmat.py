...
def readmat(datadir, winsize, wininc, pos='forearm', normalize=1):
    print('Reading data...')
    data = sio.loadmat(datadir)['DATA']    # 读取mat数据
    ...
    if pos == 'forearm':
        ...
        posdata = data
    print('Completed!')
    numsamples_sigcell = math.floor((10240-winsize)/wininc) + 1    # math.floor(x)：将 x 向下舍入到最接近的整数；numsamples_sigcell：每个cell的样本个数
    num_trial = data.shape[0]
    num_mov = data.shape[1]
    num_ch = data[0, 0].shape[1]
    samples = np.zeros((num_mov*num_trial*numsamples_sigcell, winsize, num_ch))    # 创建零矩阵
    ...
    samples = torch.from_numpy(samples)    # 把数组转换成张量
